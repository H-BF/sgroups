...
func cmpOp2string(o expr.CmpOp) string {
	vals := []string{"Eq", "Neq", "Lt", "Lte", "Gt", "Gte"}
	if o < 0 || expr.CmpOp(len(vals)) <= o {
		return "undef"
	}
	return vals[o]
}

func payloadOperationType2string(t expr.PayloadOperationType) string {
	vals := []string{"Load", "Write"}
	if t < 0 || expr.PayloadOperationType(len(vals)) <= t {
		return "undef"
	}
	return vals[t]
}

func payloadBase2string(b expr.PayloadBase) string {
	switch b {
...
func payloadCsumType(ct expr.PayloadCsumType) string {
	vals := []string{"None", "Inet"}
	if ct < 0 || expr.PayloadCsumType(len(vals)) <= ct {
		return "undef"
	}
	return vals[ct]
}
