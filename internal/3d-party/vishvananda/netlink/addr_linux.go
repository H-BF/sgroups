...
import (
	...
	"github.com/H-BF/sgroups/internal/3d-party/vishvananda/netlink/nl"
...
func addrSubscribeAt(newNs, curNs netns.NsHandle, ch chan<- AddrUpdate, done <-chan struct{}, cberr func(error), listExisting bool, rcvbuf int) error {
	...
	go func() {
		//defer fmt.Println("addr-subscribe-closed")
		defer close(ch)
			...
			*/
			...
			for _, m := range msgs {
				...
				if m.Header.Type == unix.NLMSG_ERROR {
					...
					_ = errIf(fmt.Errorf("error message: %v", syscall.Errno(-error)))
					//if cberr != nil {
				...
				if msgType != unix.RTM_NEWADDR && msgType != unix.RTM_DELADDR {
					_ = errIf(fmt.Errorf("bad message type: %d", msgType))
					//if cberr != nil {
				...
				if err != nil {
					_ = errIf(fmt.Errorf("could not parse address: %v", err))
					//if cberr != nil {
				...
				m := AddrUpdate{LinkAddress: *addr.IPNet,
					LinkIndex:   ifindex,
